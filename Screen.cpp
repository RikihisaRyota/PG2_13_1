...
Screen::Screen() {
	...
	Scroll_Fixed_Value_Y = SCREEN_HEIGHT / 2;
	Zoom = { 1.0f, 1.0f };
	Ishit = false;
};

void Screen::Scroll_update(float playerX, float playerY, float SCROLLMAXX, float SCROLLMAXY) {
	//ScreenShake.setZero();
	if (playerX >= Scroll_Fixed_Value_X && playerX < SCROLLMAXX) {
	...
	if (playerY >= Scroll_Fixed_Value_Y && playerY < SCROLLMAXY) {
